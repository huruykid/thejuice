...
/**
 * Bottom-sheet city picker (on the shared BottomSheet primitive). Searches the
 * existing `cities` table by name (typeahead) and persists the choice to
 * `profiles.city_id`. NO device GPS — this is a tag, not a location permission.
 */
const CitySheet = ({ open, onClose, onSelect, currentCityId }: Props) => {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="Pick your city" height="tall">
      ...
      <div className="sticky top-0 z-10 border-b border-border bg-background p-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg bg-muted py-2 pl-9 pr-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/40"
          />
        </div>
      </div>
        ...
        <>
          ...
          {showFreeText && (
            <button
              ...
              className="flex w-full items-center gap-3 border-t border-border px-4 py-3 text-left transition-colors hover:bg-card-hover"
            >
              <MapPin className="h-4 w-4 shrink-0 text-primary" />
              <span className="text-sm text-foreground">
                Use “<span className="font-medium">{typed}</span>”
                <span className="ml-1.5 text-xs text-muted-foreground">— my city isn’t listed</span>
              </span>
            </button>
          )}
