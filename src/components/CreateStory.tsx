...
const CreateStory = ({
  ...
  const availableTags = ["🚩Red Flag", "💯Loyal", "🫠Ghosted", "💸Gold Digger", "✨Thoughtful", "📱Phone Addict", "🤷‍♀️Mixed Signals", "🎭Fake", "🔥Chemistry", "❄️Cold", "🎵Music Lover", "🍕Foodie", "💼Career Focused", "🏃‍♂️Active", "🎨Creative"];
  const datingApps = ["Tinder", "Bumble", "Hinge", "Instagram", "IRL", "Raya", "Facebook Dating", "Coffee Meets Bagel", "OkCupid", "Match", "eHarmony", "Other"];
  const relationshipStages = ["First Date", "Hookup", "Casual Dating", "Talking Stage", "Exclusive", "LTR", "Situationship", "Friends with Benefits", "One Night Stand"];
  const prompts = ["What happened? Share your story...", "How did this experience make you feel?", "What would you want to tell your friends about this person?", "Any red flags or green flags you noticed?"];
  ...
  if (showSuccess) {
    return <div className="fixed inset-0 bg-gradient-to-br from-juice-blue/20 to-juice-coral/20 backdrop-blur-sm z-50 flex items-center justify-center">
  ...
  return <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl max-h-[90vh] overflow-hidden">
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-200px)]">
          {/* Step 0: Person Name */}
          {step === 0 && <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={storyData.personName} onChange={e => setStoryData(prev => ({
                ...prev,
                personName: e.target.value
              }))} placeholder="@instagram username" className="rounded-2xl border-juice-blue/20 focus:border-juice-blue" maxLength={50} />
                ...
                <div>
                  ...
                  <Input value={storyData.personPhone} onChange={e => setStoryData(prev => ({
                ...prev,
                personPhone: e.target.value
              }))} placeholder="e.g., +1234567890, (555) 123-4567..." className="rounded-2xl border-juice-blue/20 focus:border-juice-blue" type="tel" />
          ...
          {step === 1 && <div className="p-6 space-y-6">
              <div className="space-y-3">
                <h3 className="font-semibold text-foreground">What’s the juice?</h3>
                <p className="text-sm text-muted-foreground">
                  {prompts[0]}
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {imagePreviews.length === 0 ? <div className="border-2 border-dashed border-juice-blue/30 rounded-lg p-6 text-center hover:border-juice-blue/50 transition-smooth">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {imagePreviews.map((preview, index) => <div key={index} className="relative">
                          <img src={preview} alt={`Story preview ${index + 1}`} className="w-full h-24 object-cover rounded-lg" />
                          <Button type="button" variant="outline" size="icon" className="absolute top-1 right-1 h-6 w-6 bg-white/80 hover:bg-white" onClick={() => removeImage(index)}>
                            <Trash2 className="h-3 w-3" />
                          </Button>
                        </div>)}
                    </div>
                    
                    {imagePreviews.length < 5 && <div className="border-2 border-dashed border-juice-blue/30 rounded-lg p-4 text-center hover:border-juice-blue/50 transition-smooth">
          ...
          {step === 2 && <div className="p-6 space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {availableTags.map(tag => <Badge key={tag} variant={storyData.selectedTags.includes(tag) ? "default" : "outline"} className={`cursor-pointer transition-smooth rounded-full px-3 py-1 ${storyData.selectedTags.includes(tag) ? "bg-juice-blue text-white" : "border-juice-blue/30 text-juice-blue hover:bg-juice-blue hover:text-white"}`} onClick={() => toggleTag(tag)}>
                    {tag}
                  </Badge>)}
              </div>

              <Button variant="juice-soft" className="w-full flex items-center gap-2" onClick={() => {
          ...
          {step === 3 && <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                {Object.entries(storyData.ratings).map(([category, rating]) => <div key={category} className="space-y-2">
                    <label className="text-sm font-medium text-foreground capitalize">
                      {category === 'emotionalSafety' ? 'Emotional Safety' : category === 'overallVibe' ? 'Overall Vibe' : category}
                    </label>
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map(star => <button key={star} onClick={() => setRating(category as keyof typeof storyData.ratings, star)} className="transition-smooth">
                          <Star className={`h-6 w-6 ${star <= rating ? "fill-juice-coral text-juice-coral" : "text-muted-foreground hover:text-juice-coral"}`} />
                        </button>)}
                    </div>
                  </div>)}
              </div>
          ...
          {step === 4 && <div className="p-6 space-y-6">
              <div className="space-y-3">
                <h3 className="font-semibold text-foreground">
                  Add some context 📍
                </h3>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={storyData.metadata.city} onChange={e => setStoryData(prev => ({
                ...prev,
                metadata: {
                  ...prev.metadata,
                  city: e.target.value
                }
              }))} placeholder="e.g., Los Angeles, NYC, Miami..." className="rounded-2xl border-juice-blue/20" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={storyData.metadata.app} onValueChange={value => setStoryData(prev => ({
                ...prev,
                metadata: {
                  ...prev.metadata,
                  app: value
                    ...
                    <SelectContent className="bg-white border border-juice-blue/20 rounded-2xl shadow-lg">
                      {datingApps.map(app => <SelectItem key={app} value={app.toLowerCase()}>
                          {app}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={storyData.metadata.stage} onValueChange={value => setStoryData(prev => ({
                ...prev,
                metadata: {
                  ...prev.metadata,
                  stage: value
                    ...
                    <SelectContent className="bg-white border border-juice-blue/20 rounded-2xl shadow-lg">
                      {relationshipStages.map(stage => <SelectItem key={stage} value={stage.toLowerCase()}>
                          {stage}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
        ...
        <div className="p-6 border-t border-juice-blue/10 flex gap-3">
          {step > 0 && <Button variant="juice-outline" onClick={handleBack} className="flex-1">
              Back
            </Button>}
          {step < 4 ? <Button variant="juice" onClick={handleNext} disabled={step === 0 && !storyData.personName.trim() && !storyData.personPhone.trim() || step === 1 && !storyData.content.trim()} className="flex-1">
              {step === 3 ? "Add Details (Optional)" : "Next"}
            </Button> : <Button variant="juice" onClick={handlePublish} disabled={createStory.isPending || uploading} className="flex-1 relative overflow-hidden">
              {createStory.isPending || uploading ? <span className="flex items-center gap-2">
