-- Security Fix Phase 1: Database Security Hardening (Fixed)

-- First, check and handle null user_id values in existing data
-- For stories table - we need to handle existing null user_id values
UPDATE public.stories 
SET user_id = profile_id 
WHERE user_id IS NULL AND profile_id IS NOT NULL;

-- For comments table - we need to handle existing null user_id values  
UPDATE public.comments 
SET user_id = profile_id 
WHERE user_id IS NULL AND profile_id IS NOT NULL;

-- Delete any remaining records that can't be fixed (orphaned data)
...
-- 3. Restrict profile visibility to authenticated users only
...
-- 4. Improve invite code security - restrict SELECT to specific use cases
...
-- 5. Add rate limiting function for invite code generation
...
-- 6. Add content validation function for stories
...
-- 7. Add username validation function with reserved names
...
-- 8. Add validation triggers for stories
...
-- 9. Add validation trigger for profiles
...
-- 10. Update the username availability function to use the new validation
